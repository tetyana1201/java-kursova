...
const ExStatemetnsBlockPlusTheory = () => {
    ...
    useEffect(() => {
        const fetchSavedTheory = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, '/thexstatemetnsblock'));
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const docRef = await addDoc(collection(db, '/thexstatemetnsblock'), {
                title,
                content,
                createdAt: new Date(),
            });
    ...
    const handleEdit = async () => {
        try {
            const theoryRef = doc(db, '/thexstatemetnsblock', savedTheory.id);
    ...
    return (
        <div style={{width: '100%', height: '100%', position: 'relative', background: '#F4F2F6'}}>
            <div style={{width: 1440, height: 2534, position: 'absolute', left: 0, top: 94}}>
                <div style={{width: 1440, height: 2600, background: '#F4F2F6'}}/>
                ...
                <div style={{width: 1440, height: 397, background: 'white', position: 'absolute', left: 0, top: 2700}}/>
                <div style={{
                    color: '#150E06',
                    fontSize: 16,
                    fontFamily: 'Raleway',
            ...
            <div style={{width: '100%', height: '100%', position: 'relative', top: 2700}}>
                <Link to="/about" style={{
                    position: 'absolute',
                    left: 745,
                    top: 45,
...
export default ExStatemetnsBlockPlusTheory;